# ==============================================================================
# backend_runner.py - "The Date Fix" Engine (v38.0)
# Features: Correct Match Dates, Strict Settlement Logic, Multi-Sport
# ==============================================================================
import pandas as pd
import numpy as np
...
# ==============================================================================
# 1. SETTLEMENT ENGINE (Strict Date Checking)
# ==============================================================================
def settle_bets():
    """
    Reads history, checks results, and marks W/L/D.
    CRITICAL FIX: Only settles bets where the Match Date is in the past.
    """
    ...
    if 'Date' not in df.columns: df['Date'] = datetime.now().strftime('%Y-%m-%d') # Fallback
        ...
        try:
            ...
            for idx, row in soccer_pending.iterrows():
                try:
                    ...
                    # We look for a match with same teams within 2 days of the bet date
                    match = results[
                        (results['HomeTeam'] == home) & 
                        (results['AwayTeam'] == away) &
                        (results['Date'] >= match_date - timedelta(days=1)) &
                        (results['Date'] <= match_date + timedelta(days=2))
                    ]
        ...
        try:
            ...
            for idx, row in nfl_pending.iterrows():
                try:
                    ...
                    game = finished[(finished['home_team'] == teams[1]) & (finished['away_team'] == teams[0])].tail(1)
                    
                    if not game.empty:
                        g = game.iloc[0]
                        winner = g['home_team'] if g['home_score'] > g['away_score'] else g['away_team']
                        if 'Moneyline' in row['Bet Type']:
                            bet_team = row['Bet'].replace(' Win', '').replace('Home', teams[1]).replace('Away', teams[0]).strip()
