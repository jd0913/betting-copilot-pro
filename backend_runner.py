# ==============================================================================
# backend_runner.py - The Automated Backend Engine
# ==============================================================================
import pandas as pd
import numpy as np
...
(This is the "Cockpit." It's the fast, lightweight Streamlit dashboard that you will deploy. It simply reads and displays the results generated by the backend.)
...
def load_data():
    ...
    try:
        ...
        st.error(f"Could not load the latest bets. The backend might not have run yet, or the GitHub URL is incorrect.")
        st.info(f"Attempted to load from: {DATA_URL}")
        return pd.DataFrame()

# --- Main App Logic ---
st.header("📈 Value Dashboard")
st.markdown("These recommendations are automatically updated daily by the backend engine.")
...
if not value_df.empty:
    ...
    with col1:
        if bet_of_the_week:
            st.metric(label="🎯 Bet of the Week", value=f"{bet_of_the_week[0]['Bet']} in '{bet_of_the_week[0]['Match']}'", delta=f"Odds: {bet_of_the_week[0]['Odds']:.2f}")
        else:
            st.info("No high-confidence favorite found.")
    with col2:
        if top_underdog:
            st.metric(label="⚡ Top Underdog Play", value=f"{top_underdog[0]['Bet']} in '{top_underdog[0]['Match']}'", delta=f"Odds: {top_underdog[0]['Odds']:.2f}")
        else:
            st.info("No significant underdog value found.")
    ...
    if len(value_bets) >= 2:
        ...
        for n in [2, 3]:
            ...
            if best_parlay_combo:
                ...
                st.markdown(f"**🔥 Top {n}-Team Parlay (+EV Edge: {best_parlay_edge:.2%}) | Total Odds: {total_odds:.2f}**")
                for leg in best_parlay_combo:
                    st.markdown(f"  - **Leg:** {leg['Match']} -> **{leg['Bet']}** @ {leg['Odds']:.2f}")
    else:
        st.info("Not enough value bets to build a compelling parlay.")

else:
    st.info("No value bets are currently recommended, or the backend is running its daily analysis. Check back soon.")
