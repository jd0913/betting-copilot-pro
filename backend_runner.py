# ==============================================================================
# backend_runner.py - "The Finished Product" Engine (v45.0)
# Features: Universal Settlement, Genetic AI, Live Odds, Arbitrage, Multi-Sport
# ==============================================================================
import pandas as pd
import numpy as np
...
# ==============================================================================
# 1. UNIVERSAL SETTLEMENT ENGINE (The "Closed Loop")
# ==============================================================================
def settle_bets():
    """
    Reads history, checks results for ALL sports, and marks W/L/D.
    """
    ...
    if not soccer_pending.empty:
        try:
            # Fetch latest results (EPL example - expand for other leagues in prod)
            ...
            for idx, row in soccer_pending.iterrows():
                try:
                    ...
                    match = results[(results['HomeTeam'] == teams[0]) & (results['AwayTeam'] == teams[1])].tail(1)
                    if not match.empty and pd.notna(match.iloc[0]['FTR']):
                        res = match.iloc[0]['FTR']
                        score = f"{int(match.iloc[0]['FTHG'])}-{int(match.iloc[0]['FTAG'])}"
    ...
    if not nfl_pending.empty:
        try:
            ...
            finished_games = games[games['result'].notna()]
            
            for idx, row in nfl_pending.iterrows():
                try:
                    # Match format: "Away @ Home"
                    teams = row['Match'].split(' @ ')
                    game = finished_games[(finished_games['away_team'] == teams[0]) & (finished_games['home_team'] == teams[1])].tail(1)
                    
                    if not game.empty:
                        g = game.iloc[0]
                        score = f"{g['away_team']} {int(g['away_score'])}-{int(g['home_score'])} {g['home_team']}"
                        df.at[idx, 'Score'] = score
                        
                        # Logic for Moneyline (Simplified)
    ...
    for game in odds_data:
        ...
        if h_abbr and a_abbr:
            ...
            if bh['price'] > 0:
                ...
                if edge > 0.05: bets.append({'Date': match_time, 'Sport': 'NFL', 'League': 'NFL', 'Match': f"{game['away_team']} @ {game['home_team']}", 'Bet Type': 'Moneyline', 'Bet': 'Home Win', 'Odds': bh['price'], 'Edge': edge, 'Confidence': model_prob_home, 'Stake': (edge/(bh['price']-1))*0.25, 'Info': f"Best: {bh['book']}"})
    return pd.DataFrame(bets)
...
def run_nba_module():
    ...
    for game in odds_data:
        ...
        if model_home and model_away:
            ...
            if bh['price'] > 0:
                edge = (model_prob_home * bh['price']) - 1
                if edge > 0.05: bets.append({'Date': match_time, 'Sport': 'NBA', 'League': 'NBA', 'Match': f"{game['away_team']} @ {game['home_team']}", 'Bet Type': 'Moneyline', 'Bet': 'Home Win', 'Odds': bh['price'], 'Edge': edge, 'Confidence': model_prob_home, 'Stake': (edge/(bh['price']-1))*0.25, 'Info': f"Best: {bh['book']}"})
    return pd.DataFrame(bets)
