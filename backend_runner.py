# ==============================================================================
# backend_runner.py - "The Closed Loop" Engine (v36.0)
# Features: Auto-Settlement, P&L Tracking, Feedback Loop, All Previous Features
# ==============================================================================
import pandas as pd
import numpy as np
...
# ==============================================================================
# 1. THE SETTLEMENT ENGINE (NEW)
# ==============================================================================
def settle_bets():
    """
    Reads betting_history.csv, checks for Pending bets, fetches scores, 
    and updates the file with Results (Win/Loss) and Profit.
    """
    print("--- ⚖️ Running Settlement Engine ---")
    history_file = 'betting_history.csv'
    if not os.path.exists(history_file):
        print("   > No history file found. Skipping settlement.")
        return 0.0 # Return 0 ROI
    ...
    if pending.empty:
        ...
        return calculate_recent_roi(df)

    print(f"   > Found {len(pending)} pending bets. Fetching scores...")
    ...
    if not soccer_pending.empty:
        try:
            ...
            seasons = ['2324'] # Current season
            results = pd.concat([pd.read_csv(f'https://www.football-data.co.uk/mmz4281/{s}/E0.csv', encoding='latin1') for s in seasons])
            # (In a full version, you'd loop through all leagues. Simplified for E0 here)
            
            for idx, row in soccer_pending.iterrows():
                teams = row['Match'].split(' vs ')
                home, away = teams[0], teams[1]
                
                # Find match in results (fuzzy match date/teams)
                ...
                if not match.empty and pd.notna(match.iloc[0]['FTR']):
                    ...
                    score_str = f"{int(result_row['FTHG'])}-{int(result_row['FTAG'])}"
                    df.at[idx, 'Score'] = score_str
                    
                    won = False
                    if row['Bet'] == 'Home Win' and result_row['FTR'] == 'H': won = True
                    elif row['Bet'] == 'Draw' and result_row['FTR'] == 'D': won = True
                    elif row['Bet'] == 'Away Win' and result_row['FTR'] == 'A': won = True
                    elif row['Bet Type'] == 'BTTS' and row['Bet'] == 'Yes' and result_row['FTHG'] > 0 and result_row['FTAG'] > 0: won = True
    ...
    if not nfl_pending.empty:
        try:
            scores = nfl.import_schedules(years=[2023, 2024])
            scores = scores[scores['result'].notna()] # Only completed games
            ...
            for idx, row in nfl_pending.iterrows():
                ...
                # Find game
                game = scores[(scores['home_team'] == home) & (scores['away_team'] == away)].tail(1)
                
                if not game.empty:
                    g = game.iloc[0]
                    score_str = f"{g['away_team']} {int(g['away_score'])} - {g['home_team']} {int(g['home_score'])}"
                    df.at[idx, 'Score'] = score_str
                    
                    # Parse Bet: "KC -3.0"
                    bet_parts = row['Bet'].split(' ')
                    bet_team = bet_parts[0]
                    bet_line = float(bet_parts[1])
                    
                    margin = g['home_score'] - g['away_score'] if bet_team == home else g['away_score'] - g['home_score']
                    
                    if margin > (bet_line * -1): # Covered spread
                        df.at[idx, 'Result'] = 'Win'
                        df.at[idx, 'Profit'] = row['Stake'] * (row['Odds'] - 1)
                    elif margin == (bet_line * -1):
...
def calculate_recent_roi(df):
    """Calculates ROI of the last 20 settled bets to adjust strategy."""
    settled = df[df['Result'].isin(['Win', 'Loss'])]
    if settled.empty: return 0.0
    
    recent = settled.tail(20)
    total_staked = recent['Stake'].sum() # Assuming stake is unit size or %
    total_profit = recent['Profit'].sum()
    
    if total_staked == 0: return 0.0
    roi = total_profit / total_staked
    print(f"   > Recent Performance (Last 20): ROI {roi:.2%}")
    return roi

# ==============================================================================
# 2. THE DARWIN MODULE (Unchanged)
# ==============================================================================
...
def get_live_odds(sport_key):
    ...
    url = f'https://api.the-odds-api.com/v4/sports/{sport_key}/odds'; params = {'api_key': API_CONFIG["THE_ODDS_API_KEY"], 'regions': 'us', 'markets': 'h2h,spreads', 'oddsFormat': 'decimal'}
    try:
...
def run_soccer_module(brain, historical_df):
    print("--- Running Soccer Module (US Pro) ---"); bets = []; odds_data = get_live_odds('soccer_epl')
    elo_ratings = brain['elo_ratings']; team_strengths = brain['team_strengths']; avg_goals_home, avg_goals_away = brain['avgs']
    for game in odds_data:
        profit, arb_info, bh, bd, ba = find_arbitrage(game, 'Soccer')
        if profit > 0: bets.append({'Sport': 'Soccer', 'League': 'EPL', 'Match': f"{game['home_team']} vs {game['away_team']}", 'Bet Type': 'ARBITRAGE', 'Bet': 'ALL', 'Odds': 0.0, 'Edge': profit, 'Confidence': 1.0, 'Stake': 0.0, 'Info': arb_info}); continue
        model_home = fuzzy_match_team(game['home_team'], list(elo_ratings.keys())); model_away = fuzzy_match_team(game['away_team'], list(elo_ratings.keys()))
        if model_home and model_away:
            ...
            if bh['price'] > 0:
                ...
                if edge > 0.05: bets.append({'Sport': 'Soccer', 'League': 'EPL', 'Match': f"{game['home_team']} vs {game['away_team']}", 'Bet Type': 'Moneyline', 'Bet': 'Home Win', 'Odds': bh['price'], 'Edge': edge, 'Confidence': final_probs['Home Win'], 'Stake': (edge/(bh['price']-1))*0.25*vol_factor, 'Info': f"Best: {bh['book']}"})
            if bd['price'] > 0:
                edge = (final_probs['Draw'] * bd['price']) - 1
                if edge > 0.05: bets.append({'Sport': 'Soccer', 'League': 'EPL', 'Match': f"{game['home_team']} vs {game['away_team']}", 'Bet Type': 'Moneyline', 'Bet': 'Draw', 'Odds': bd['price'], 'Edge': edge, 'Confidence': final_probs['Draw'], 'Stake': (edge/(bd['price']-1))*0.25*vol_factor, 'Info': f"Best: {bd['book']}"})
            if ba['price'] > 0:
                edge = (final_probs['Away Win'] * ba['price']) - 1
                if edge > 0.05: bets.append({'Sport': 'Soccer', 'League': 'EPL', 'Match': f"{game['home_team']} vs {game['away_team']}", 'Bet Type': 'Moneyline', 'Bet': 'Away Win', 'Odds': ba['price'], 'Edge': edge, 'Confidence': final_probs['Away Win'], 'Stake': (edge/(ba['price']-1))*0.25*vol_factor, 'Info': f"Best: {ba['book']}"})
...
# ==============================================================================
def run_nfl_module():
    print("--- Running NFL Module (US Pro) ---"); bets = []; odds_data = get_live_odds('americanfootball_nfl')
    ...
    for game in odds_data:
        ...
        if profit > 0: bets.append({'Sport': 'NFL', 'League': 'NFL', 'Match': f"{game['away_team']} @ {game['home_team']}", 'Bet Type': 'ARBITRAGE', 'Bet': 'ALL', 'Odds': 0.0, 'Edge': profit, 'Confidence': 1.0, 'Stake': 0.0, 'Info': arb_info}); continue
        ...
        if h_abbr and a_abbr:
            ...
            if bh['price'] > 0:
                ...
                if edge > 0.05: bets.append({'Sport': 'NFL', 'League': 'NFL', 'Match': f"{game['away_team']} @ {game['home_team']}", 'Bet Type': 'Moneyline', 'Bet': 'Home Win', 'Odds': bh['price'], 'Edge': edge, 'Confidence': model_prob_home, 'Stake': (edge/(bh['price']-1))*0.25, 'Info': f"Best: {bh['book']}"})
    return pd.DataFrame(bets)
...
# ==============================================================================
def run_nba_module():
    print("--- Running NBA Module (US Pro) ---"); bets = []; odds_data = get_live_odds('basketball_nba'); team_power = {}; team_names = []
    ...
    except Exception as e: print(f"NBA Stats Error: {e}")
    for game in odds_data:
        profit, arb_info, bh, _, ba = find_arbitrage(game, 'NBA')
        if profit > 0: bets.append({'Sport': 'NBA', 'League': 'NBA', 'Match': f"{game['away_team']} @ {game['home_team']}", 'Bet Type': 'ARBITRAGE', 'Bet': 'ALL', 'Odds': 0.0, 'Edge': profit, 'Confidence': 1.0, 'Stake': 0.0, 'Info': arb_info}); continue
        ...
        if model_home and model_away:
            ...
            if bh['price'] > 0:
                edge = (model_prob_home * bh['price']) - 1
                if edge > 0.05: bets.append({'Sport': 'NBA', 'League': 'NBA', 'Match': f"{game['away_team']} @ {game['home_team']}", 'Bet Type': 'Moneyline', 'Bet': 'Home Win', 'Odds': bh['price'], 'Edge': edge, 'Confidence': model_prob_home, 'Stake': (edge/(bh['price']-1))*0.25, 'Info': f"Best: {bh['book']}"})
    return pd.DataFrame(bets)

# ==============================================================================
# 6. NOTIFICATION & MAIN EXECUTION
# ==============================================================================
...
def run_backend_analysis():
    ...
    # 1. SETTLEMENT (The Feedback Loop)
    recent_roi = settle_bets()
    
    # Adjust stakes based on recent performance (Dynamic Kelly)
    # If ROI is positive, we are confident (1.0x). If negative, we reduce risk (0.5x).
    performance_multiplier = 1.0 if recent_roi >= 0 else 0.5
    print(f"   > Performance Multiplier: {performance_multiplier}x (Based on ROI: {recent_roi:.2%})")
    ...
    if not all_bets.empty:
        ...
        # Apply Performance Multiplier to Stake
        if 'Stake' in all_bets.columns:
            all_bets['Stake'] = all_bets['Stake'] * performance_multiplier
