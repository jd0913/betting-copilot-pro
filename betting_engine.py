...
def get_news_alert(team1, team2):
    try:
        ...
        for h in headlines[:3]:
            if any(k in h.text.lower() for k in ['injury', 'doubt', 'out', 'miss']):
...
def run_soccer_module(brain, historical_df):
    ...
    for sport_key, div_code in config.SOCCER_LEAGUES.items():
...
def settle_bets():
        ...
        try:
            ...
            for idx, row in nfl_pending.iterrows():
                try:
                    ...
                    # Map names if needed, simplified here
    ...
    # 3. NBA Settlement (Placeholder for API logic)
    # Requires live scoreboard access which is complex in free tier, skipping for stability
